...
export default function VoiceAssistedLogin() {
  ...
  useEffect(() => {
    if (step === 'greeting') {
      const msg = "Let's sign you in. Speak your email address."
      speakAndCaption(msg)
      const timer = setTimeout(() => setStep('ask_email'), 2500)
  ...
  useEffect(() => {
    if (step === 'ask_email') {
      ...
      speakAndCaption(msg)
      if (isSupported) {
        emailTimeoutRef.current = setTimeout(() => start(), 1200)
      }
      return () => {
  ...
  useEffect(() => {
    if (transcript && step === 'ask_email') {
      const cleaned = normalizeEmailTranscript(transcript)
      if (!cleaned || !cleaned.includes('@')) return

      setEmail(cleaned)
      setEmailInput(cleaned)
      stop()
      const msg = `I heard ${cleaned}. Is that correct?`
      speakAndCaption(msg)
  ...
  }, [transcript, step, stop, speakAndCaption, isSupported, start])

  // Voice handler for confirmation and choice steps
  useEffect(() => {
    ...
    if (step === 'confirm_email') {
      if (/^(yes|yeah|correct|right|yep|sure|okay|ok|confirm)$/i.test(t)) {
        ...
        /^(no|nope|wrong|incorrect|try again|repeat|again)$/i.test(t)
      ) {
        stop()
        handleEmailRetry()
      }
      ...
      if (/google/i.test(t)) {
        stop()
        handleGoogleChoice()
      } else if (/magic\s*link|magic|email/i.test(t)) {
        stop()
        handleMagicLinkChoice()
      }
    }
  }, [transcript, step])
  ...
  const handleMagicLinkChoice = async () => {
    ...
    try {
      ...
      speakAndCaption('A secure login link has been sent to your email.')
            ...
            <div className="rounded-2xl border border-zinc-200/50 bg-white/80 p-8 shadow-2xl backdrop-blur-xl dark:border-zinc-800/50 dark:bg-zinc-950/80">
              <div className="mb-6 text-center">
                <h2 className="text-2xl font-bold">
                  Choose Method for {email}
                </h2>
                <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
                  Passwords are never entered through voice. Pick a secure
                  method below.
                </p>
              </div>
